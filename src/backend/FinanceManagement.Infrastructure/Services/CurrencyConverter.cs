...
public class CurrencyConverter(
    ...
    IMemoryCache cache,
    ILogger<CurrencyConverter> logger) : ICurrencyConverter
{
    internal const string _httpClientName = "NbuCurrencyConverter";
    private const string cacheKeyPrefix = "NBU_RATES";
    ...
    private ValueTask<IReadOnlyDictionary<string, decimal>> GetRatesAsync(DateOnly date, CancellationToken ct)
    {
        ...
        if (cache.TryGetValue(cacheKey, out IReadOnlyDictionary<string, decimal>? cached) && cached is not null)
        {
    ...
    private async Task<IReadOnlyDictionary<string, decimal>> FetchRatesAsync(DateOnly date, CancellationToken ct)
    {
        ...
        {
            logger.LogError(ex, "Failed to fetch NBU rates for {Date}.", date);
    ...
    private static string BuildCacheKey(DateOnly date) => $"{cacheKeyPrefix}:{date:yyyyMMdd}";
